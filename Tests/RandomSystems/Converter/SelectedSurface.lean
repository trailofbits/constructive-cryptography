...
/-!
# Query-indexed converter tests

This module verifies the public query-indexed converter root.
-/

namespace RandomSystems.Ambient.Tests.SelectedSurface
...
/-! ## Scoped deterministic-converter category -/

/-- A DDC enters the scoped category through the explicit readable boundary. -/
noncomputable example {A B : Interface.{u, v}}
    (converter : DDC A B) : A ⟶ B :=
  converter.asHom

/-- The scoped categorical action is direct DDC attachment. -/
noncomputable example {A B : Interface.{u, v}}
    (converter : DDC A B) (system : RandomSystem B) :
    converter.asHom • system = RandomSystem.applyDDC converter system :=
  rfl

inductive TableQuery
  | first
  | second

/-- A stateful DDS can be written directly as a recursive Lean function on
its complete attempted-query history. -/
def tableSystem : DDS (Interface.single TableQuery Bool) :=
  fun history =>
...
noncomputable def atMostTwoQueries :
...
abbrev Outer := Interface.single Nat Unit
abbrev Inner := Interface.single Unit Unit

/-- The outer query `n` selects exactly `n` consecutive inner queries. -/
noncomputable def raw : DDC.Raw Outer Inner :=
  fun history =>
...
theorem raw_complete : DDC.Raw.Complete raw := by
  intro history _
  unfold raw
  split <;> exact trivial

theorem raw_branchFinite : DDC.Raw.BranchFinite raw := by
...
noncomputable def converter : DDC Outer Inner :=
  DDC.ofRaw raw raw_complete raw_branchFinite

def history (limit : Nat) : Nat → DDC.History Outer Inner
  | 0 => DDC.History.singleton limit
  | depth + 1 => (history limit depth).snocInner () none

@[simp]
theorem innerDepth_history (limit depth : Nat) :
...
@[simp]
theorem lastOuter_history (limit depth : Nat) :
...
theorem history_admissible (limit depth : Nat)
    (atMost : depth ≤ limit) :
...
abbrev FlatReply := Option Route

def replyMatchesQuery : Route → FlatReply → Prop
  | _, none => True
  | expected, some actual => actual = expected

abbrev FlatHistory := List (Unit ⊕ FlatReply)

def sameHistory : FlatHistory :=
...
end RandomSystems.Ambient.Tests.SelectedSurface
