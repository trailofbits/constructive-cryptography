...
namespace ConstructiveCryptography.Tests.Composition
...
example {A B D E : C} {π₁ : D ⟶ E} {π₂ : B ⟶ D} {π₃ : A ⟶ B}
    {key : ResourceAlgebra.Specification Phi E} {insecure : ResourceAlgebra.Specification Phi D}
    {authenticated : ResourceAlgebra.Specification Phi B} {secure : ResourceAlgebra.Specification Phi A}
...
example {A B D E : C}
    ...
    {source : ResourceAlgebra.Specification Phi E} {firstMiddle : ResourceAlgebra.Specification Phi D}
    {secondMiddle : ResourceAlgebra.Specification Phi B} {target : ResourceAlgebra.Specification Phi A}
    (first : ResourceAlgebra.Specification.Constructible (Phi := Phi)
      firstConstructors source firstMiddle)
    (second : ResourceAlgebra.Specification.Constructible (Phi := Phi)
      secondConstructors firstMiddle secondMiddle)
    (third : ResourceAlgebra.Specification.Constructible (Phi := Phi)
      thirdConstructors secondMiddle target) :
    ResourceAlgebra.Specification.Constructible (Phi := Phi)
      allConstructors source target := by
  -- Compose the first two admitted constructor classes.
  have firstTwo := ResourceAlgebra.Specification.Constructible.serial
    firstTwoAdmitted first second
  -- Compose their result with the third admitted constructor class.
  exact ResourceAlgebra.Specification.Constructible.serial allAdmitted firstTwo third
...
end ConstructiveCryptography.Tests.Composition
