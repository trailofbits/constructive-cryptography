...
This module compares an ordinary proof structure with a local
...
namespace ConstructiveCryptography.Categorical.ResourceAlgebra.Tests.ConstructionWorkflow
...
theorem ConstructionProof.constructs {A B D : C}
  ...
  cc_compose proof.inner, proof.outer
...
def problemOne : ConstructionProblem Phi A B D where
  source := source
  middle := middleOne
  target := target
...
def problemTwo : ConstructionProblem Phi A B D where
  source := source
  middle := middleTwo
  target := target
...
def incompleteProof : ConstructionProof
    (problemOne source middleOne target innerConverterOne outerConverterOne
      innerErrorOne outerErrorOne) where
  inner := innerOne
...
end ConstructiveCryptography.Categorical.ResourceAlgebra.Tests.ConstructionWorkflow
