...
-/
import AbstractCryptography.Categorical.ResourceAlgebra.Relaxation
...
noncomputable def epsilonRelaxation {A : C} (error : ENNReal)
  ...
  AbstractCryptography.Relaxation.ofPointwise
    (fun center =>
      {resource | distance (Phi := Phi) resource center ≤ error})
    (fun resource =>
      (le_of_eq (distance_self resource)).trans (zero_le error))
...
theorem mem_epsilonRelaxation_iff {A : C} {error : ENNReal}
  ...
  AbstractCryptography.Relaxation.mem_ofPointwise_iff
