...
instance [Par Φ] : Par (Set Φ) where
  par R S := Set.image2 (· ∥ ·) R S

theorem mem_par_iff [Par Φ] {R S : Set Φ} {x : Φ} :
    x ∈ R ∥ S ↔ ∃ r ∈ R, ∃ s ∈ S, r ∥ s = x := Set.mem_image2

theorem par_mem_par [Par Φ] {R S : Set Φ} {r s : Φ} (hr : r ∈ R) (hs : s ∈ S) :
...
instance [Par Φ] [Par M] [SMulParClass M Φ] : IsContextInsensitive (Set Φ) M where
...
theorem Constructs.par_left [Par Φ] [Par M] [SMulParClass M Φ]
    {π : M} {R S : Set Φ} (T : Set Φ) (h : R —[π]→ S) :
    R ∥ T —[π ∥ (1 : M)]→ S ∥ T :=
  red_par_one T h

/-- `Constructs.par_left` for singleton specifications. -/
theorem Constructs.par_left_resource [Par Φ] [Par M] [SMulParClass M Φ]
    {π : M} {R S : Φ} (T : Φ)
    (h : ({R} : Set Φ) —[π]→ ({S} : Set Φ)) :
    ({R ∥ T} : Set Φ) —[π ∥ (1 : M)]→ ({S ∥ T} : Set Φ) := by
  simpa only [singleton_par_singleton] using h.par_left ({T} : Set Φ)
...
instance [Par Φ] [Par M] [SMulParClass M Φ] : IsGenerallyComposable (Set Φ) M where
...
out: `π ∥ 1` and `σ ∥ 1`. -/
theorem Constructs.simulator_par {π σ : M} {R S : Set Φ} (T : Set Φ)
    [Par Φ] [Par M] [SMulParClass M Φ]
