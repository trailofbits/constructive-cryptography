...
/-!
# Controlled natural language for Constructive Cryptography

This module is the paper-facing syntax layer above
`ConstructiveCryptography.Tactics.ProofAutomation`.  Its design follows the three-layer
...
Every sentence expands to one existing `cc_*` command, so the ideal resource,
simulator, intermediate specification, error budget, and proof route remain
explicit.

The syntax is scoped.  Importing `ConstructiveCryptography` makes the implementation
...
concrete vocabulary to CC.

## Corpus-derived language profile

The wording is taken from the recurring proof prose of the constructive-
cryptography and random-systems papers themselves. Verbose Lean and
ForTheL/Naproche motivate the
...
names such as `cc.construction.by_composition`; downstream extensions should
use `cc.` or `rs.` prefixes. -/
initialize Lean.registerTraceClass `CryptoControlledNaturalLanguage.sentence

/-- Shared lowering boundary for CC and concrete controlled-language
...
named fact.  The noun selects the corresponding deterministic CC rule; no
...
scoped macro (name := cnlFollowsFrom)
  ...
  | "construction" =>
      `(tactic|
        crypto_cnl_sentence "cc.construction.follows_from" =>
          cc_construct using $fact)
  | "equality" =>
      `(tactic|
        crypto_cnl_sentence "cc.equality.follows_from" =>
          cc_transport using $fact)
  | _ =>
...
scoped macro (name := cnlReplaceConverter)
  ...
  `(tactic|
    crypto_cnl_sentence "cc.construction.replace_converter" =>
      cc_transport $construction using $equation)
...
scoped macro (name := cnlComposeConstruction)
  ...
  `(tactic|
    crypto_cnl_sentence "cc.construction.by_composition" =>
      cc_compose $firstConstruction, $secondConstruction)
...
scoped macro (name := cnlHaveFact)
  ...
  `(tactic|
    crypto_cnl_sentence "cc.argument.named_fact" =>
...
scoped macro (name := cnlUseSimulator)
  ...
  `(tactic|
    crypto_cnl_sentence "cc.construction.from_simulator" =>
...
scoped macro (name := cnlParallelContext)
  ...
  | "right" =>
      `(tactic|
        crypto_cnl_sentence "cc.construction.right_parallel_context" =>
          cc_context_left $context using $construction)
  | "left" =>
      `(tactic|
        crypto_cnl_sentence "cc.construction.left_parallel_context" =>
