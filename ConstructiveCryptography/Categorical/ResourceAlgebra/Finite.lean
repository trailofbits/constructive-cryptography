...
def specifications : (n : Nat) → (interfaces : Fin n → C) →
  ...
  | 0, _, _ =>
      ConstructiveCryptography.Categorical.ResourceAlgebra.Specification.dummy
        (C := C) (Phi := Phi)
  | n + 1, interfaces, family =>
      ConstructiveCryptography.Categorical.ResourceAlgebra.Specification.parallel
        (Phi := Phi) (family 0)
...
theorem constructs (n : Nat) (outer inner : Fin n → C)
  ...
  | zero =>
      ...
      exact ConstructiveCryptography.Categorical.ResourceAlgebra.Specification.constructs_identity _
  | succ n inductionHypothesis =>
      -- Compose the first construction in parallel with the induction result.
      exact ConstructiveCryptography.Categorical.ResourceAlgebra.Specification.Constructs.parallel
        (construction 0)
        (inductionHypothesis (fun i => outer i.succ)
          (fun i => inner i.succ) (fun i => family i.succ)
...
theorem constructs_within (n : Nat) (outer inner : Fin n → C)
    ...
    (construction : ∀ i,
      ConstructiveCryptography.Categorical.ResourceAlgebra.Specification.ConstructsWithin
        (Phi := Phi)
  ...
  | zero =>
      ...
      exact
        ConstructiveCryptography.Categorical.ResourceAlgebra.Specification.constructsWithin_identity _
  | succ n inductionHypothesis =>
      -- Binary approximate parallel adds the first and remaining errors.
      exact ConstructiveCryptography.Categorical.ResourceAlgebra.Specification.ConstructsWithin.parallel
        (construction 0)
        (inductionHypothesis (fun i => outer i.succ)
          (fun i => inner i.succ) (fun i => family i.succ)
          (fun i => source i.succ) (fun i => target i.succ)
