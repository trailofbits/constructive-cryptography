...
-/
import ConstructiveCryptography.Categorical
...
locality. The monoidal and lax-monoidal structures are this library's packaging
of the proved ordered associativity, unit, and locality equations.
...
abbrev Specification (Phi : Opposite C ⥤ Type w) (A : C) :=
  ConstructiveCryptography.Specification (Resource Phi A)
...
noncomputable instance instHSMulConverterResource {A B : C} :
    HSMul (A ⟶ B) (Resource Phi B) (Resource Phi A) where
  hSMul := attach (Phi := Phi)

omit [MonoidalCategory C] in
@[simp]
theorem smul_eq_attach {A B : C} (converter : A ⟶ B)
...
Jost, Section 2.2.2 (printed p. 17): “The resource consisting of the dummy
