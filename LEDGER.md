...
| T1.7 | **CE ⟹ indistinguishability, adaptive** `Adv ≤ Γ(Ŝ)` | Maurer02 Thm 1; CR18 Thm 4.17 | `Q:Theorem417.lean:764` (and `:724`); delta `both` | none; target distance `Adv⊥` exists (`Environment.lean:751`) | CLOSED (T3 b82f449, renamed 84772d0 per the paper-identity rule: `PDG.conditional_equivalence_theorem` (+ `_adjoin`/`_gamesFor`/`_infWinnability` forms), stated through `ν[G]`; audited bundle NonNeg×2 + equal weight only — quarry nodes T1.4/T1.6/monotone-bridge collapse on this carrier, audit 2.3 confirms no silent weakening). Was: **TRANSPLANT** — restate on `Adv⊥`; the hypothesis bundle re-derives (see C2/C3) | L |
| T1.8 | **CE ⟹ indistinguishability, blind** `Δ ≤ Γ(bŜ)` (the headline) | **Maurer13b Thm 3, printed p. 3154** (via `⟦DT⟧`); **CR18 Thm 4.17, printed p. 110** (via the blocking converter `b`, Def 4.20 p. 109) — both VERIFIED, both NON-ADAPTIVE RHS unconditionally | `Q:GameOf.lean:1472`, `:1431`; delta `both` | none | **TRANSPLANT** — the strictly stronger endpoint; needs T3.2.  **See the `b`-as-converter note below: AC can state this paper-faithfully where the reference repository could not** | **CLOSED 2026-08-19** (aeb685f + audit fix-up e543b5f; adversarially audited — mathematics SURVIVED in full).  Blind half: `PDG.conditional_equivalence_theorem_blind` (BlindWinning.lean) by the PAPER's route through the enhanced system, and `advFullyDefined_forget_le_blindSupWinProb_of_condEquiv` — the SAME bound with NEITHER the normalization nor the domain clause (kernel receipt `conditional_equivalence_theorem_blind_subsumed`: the headline is a strict instance of it).  The extra hypotheses belong to the enhanced-system construction that `GameRelaxation` consumes, NOT to the bound; the domain clause is a carrier artefact (Maurer13b has no refusals at all).  CAVEAT (2) RESOLVED: CR18's `b` is REPLY-side, an ordinary `converterMonoidAt` member — no widening was needed.  CAVEAT (4) REFUTED at the whole face: the reply-eraser answers its constant even to a refusal, so composing it with ANY resource yields the constant-answer system (`attachEngineFully_blockReplies_univ`), and the two renderings coincide (`supWinProb_blockRepliesGame`, both directions); at a PROPER subinterface refusals outside it remain observable — scope recorded in BlockReplies.lean.  NEW: blind is STRICTLY weaker, now a theorem not a belief (`exists_blindSupWinProb_lt_supWinProb`, secret-bit game).  NOTE: `enhance` is NOT eq. (4.39) — the enhancement display is unnumbered; (4.39) is `Ŝ ≡ᵍ T̂` (7 mislabels corrected at e543b5f) |
...
this date.  OWED-ITEM STATUS 2026-08-19: the repo-wide §3.4.3 citation fix is
DISCHARGED (e543b5f, ~15 sites incl. the check-5 comment; sites genuinely
meaning the `[q]` notation keep Def 3.10).  STILL OWED: extend the pin to
`filterPhi` — the pinned text delegates the family's meaning to an unpinned
definition; coordinator recommendation is to do it (pure hardening, no
mathematical change, costs a second stored hash).
...
so the surface is visible; wrong pages are caught only by reading the rendered
page, so every brief keeps the visual-read mandate; R11(b) minimal migration — where a proven
...
FLAG **F-8 RETIRED 2026-08-19** (aeb685f/e543b5f).  Its PREMISE was wrong —
the escalation conflated query-side `block` with CR18 Def 4.20's reply-side
`b` — and CR18 Lemma 5.3 is now built (`GameRelaxation.lean`).  The flag was
NOT spurious: it correctly identified that a construction was required, and
that construction (`enhance`, `blockRepliesGame`) was then built WITHOUT the
fork the flag called for.  COORDINATOR RULING, recorded and flagged for Marc:
a PDG assembled from landed pieces on the unchanged carrier (a product law
plus a condition transported by `comap`) is a DEFINITION OVER EXISTING
OBJECTS and so admissible under R11(a); what R11(a) forbids is a new object
STACK or a source-presentation OPERATOR (`S⁻`/`S⊣`, `Γ`/`Γᵇ`, a blinded-system
object).  `b` is none of those.
