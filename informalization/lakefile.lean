...
/-- The live abstract-crypto checkout.  This is a path dependency so the
language project follows the user's evolving working tree. -/
require ConstructiveCryptography from "../../abstract-crypto"

/-- The live CBC consumer. Its public theorem is an integration fixture for
the shared Random Systems ontology plus the thin CBC vocabulary extension. -/
require «cbc-mac-cc» from "../../cbc-mac-cc"
...
private def validateUpstream : IO UInt32 := do
  let upstream := "../../abstract-crypto"
  let cbcConsumer := "../../cbc-mac-cc"
  ...
  if localToolchain != upstreamToolchain then
    IO.eprintln s!"Lean toolchain mismatch: this project uses {localToolchain}, but the live abstract-crypto checkout uses {upstreamToolchain}"
    return 1
  if localToolchain != cbcToolchain then
    IO.eprintln s!"Lean toolchain mismatch: this project uses {localToolchain}, but the live cbc-mac-cc checkout uses {cbcToolchain}"
    return 1
  ...
  IO.println s!"abstract-crypto upstream: {rootResult.stdout.trimAscii}"
  IO.println s!"abstract-crypto commit: {commitResult.stdout.trimAscii} ({state})"
  IO.println s!"cbc-mac-cc consumer: {cbcConsumer}"
