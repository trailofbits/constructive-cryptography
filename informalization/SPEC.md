...
The language tooling is its own Lake project in the `informalization/`
directory of the `constructive-cryptography` repository.  It depends on the
enclosing checkout through a path dependency on its parent directory, so it
always builds against the library commit it is committed with:
...
The project depends on the enclosing `constructive-cryptography` checkout and,
as a git dependency pinned in `lake-manifest.json`, on the `cbc-mac-cc`
consumer. They make the declarations and compiled proof terms available. That is sufficient for
...
1. resolve `CBCMAC/Main.lean` of the materialized `cbc-mac-cc` package with
   `lake setup-file`, run from this workspace;
