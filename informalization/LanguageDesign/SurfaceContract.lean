...
def informalizationCollisionEquivalenceReduction : CanonicalForm :=
  canonical `informalization.collisionEquivalenceReduction
    rsConditionalBlindBound #[
      literal "Following CR18, augment CBC with the monotone binary output \\(A=(A_1,A_2,\\ldots)\\). Here \\(A_i=1\\) exactly when, up to the evaluation of the \\(i\\)-th message, a non-trivial collision has occurred at an input to the round function. A collision caused only by two messages sharing a prefix is not counted. If \\(A_i=0\\), prefix-freeness makes the final round-function inputs distinct for distinct messages. Conditioned on \\(A_i=0\\), the CBC outputs are uniform on distinct messages, while repeated messages receive the same output. Therefore the game and the ideal system satisfy CR18's central conditional-equivalence step, equation (6.2):",
      roleHole `baseConditionalEquivalence,
      literal "Applying the total-block restriction to both sides gives",
      roleHole `restrictedConditionalEquivalence,
      literal "Together with the converter-application identity above, Theorem 4.17 bounds the original distance by the probability of winning the restricted collision game blindly:",
      roleHole `consequence]
