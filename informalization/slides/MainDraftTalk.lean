...
def main : IO UInt32 := do
  ...
  let rc ← slidesMain (config := config) (doc := hoistFootnotes (%doc DraftTalk))
  ...
  unless slideJs.contains "sectionTag: 'article'" do
    throw <| IO.userError "the Verso adapter allows reader sections to become Reveal slides"
  unless slideJs.contains "window.Reveal.on('ready', mountCurrent)" &&
      slideJs.contains "window.Reveal.on('slidechanged', mountCurrent)" &&
      slideJs.contains "window.requestAnimationFrame" &&
      slideJs.contains "retryReaderRuntime(host)" &&
      slideJs.contains "new MutationObserver" do
    throw <| IO.userError "the Verso adapter no longer defers the proof reader until Reveal is ready"
  if slideJs.contains "DOMContentLoaded', mountAll" then
    throw <| IO.userError "the Verso adapter regressed to eagerly mounting the hidden proof"
  ...
  unless slideJs.contains "markProofMilestones(host)" &&
      slideJs.contains "dataset.proofMilestone" &&
      slideJs.contains "window.Reveal.on('fragmentshown', fragmentChanged)" &&
      slideJs.contains "window.Reveal.on('fragmenthidden', fragmentChanged)" do
    throw <| IO.userError "the Verso adapter lost the proof-milestone highlights"
  ...
  unless nativeCss.contains ".proof-milestone" &&
      nativeCss.contains "border: 0.07em dashed transparent" &&
      nativeCss.contains ".proof-milestone.is-active" do
    throw <| IO.userError "the Verso reader lost the proof-milestone styling"
  ...
  unless html.contains "data-proof-milestone-step=\"2\"" &&
      html.contains "data-proof-milestone-step=\"3\"" &&
      html.contains "data-proof-milestone-step=\"4\"" &&
      html.contains "data-proof-milestone-step=\"5\"" do
    throw <| IO.userError "the CBC slide did not render all proof-milestone fragments"
