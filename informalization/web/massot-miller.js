(function () {
  ...
  var sectionTag = options.sectionTag === 'article' ? 'article' : 'section';
  ...
  function appendLeanGoal(parent, goal) {
    if (!goal) return;
    var section = elt(sectionTag, 'lean-evidence-goal');
  ...
  function renderLeanEvidence(node, builder, context) {
    ...
    (node.evidence || []).forEach(function (evidence, index) {
      var entry = elt(sectionTag, 'lean-evidence-entry');
  ...
  function theoremNode(decl, index) {
    var theorem = elt(sectionTag, 'theorem');
