...
/-- Encode exact source for the HTML renderer without pretending that the
source is LaTeX. The JSON string makes arbitrary identifiers, delimiters, and
line breaks inert until the renderer decodes the marker. -/
def rawSourceLatex (source : String) : String :=
  "\\informalizationRaw{" ++ (Lean.Json.str source).compress ++ "}"

/-- The failsafe expression printer. It retains the ordinary pretty-printed
expression as inert source rather than inserting it into a LaTeX command. -/
def defaultLatexPP (e : Expr) : LatexPrinterM LatexData := do
  let f ← PrettyPrinter.ppExpr e
  return LatexData.ofString <| rawSourceLatex s!"{f}"
