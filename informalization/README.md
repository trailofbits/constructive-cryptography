...
The project directly imports the sibling `cbc-mac-cc` package and tests the
...
surface. Set
`INFORMALIZATION_CBC_ROOT` only to override the default sibling checkout.
...
`lake run provenance` verifies that this package and its live path dependency
use the same Lean toolchain, then reports the resolved upstream checkout,
commit, and clean/dirty state. `lake run test` performs the same gate before
