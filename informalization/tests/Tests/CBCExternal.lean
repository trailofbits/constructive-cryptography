...
By default the project is the `cbc-mac-cc` dependency that Lake materializes
under `.lake/packages/cbc-mac-cc`; the `INFORMALIZATION_CBC_ROOT` environment
variable may override that path with another checkout that belongs to this
workspace. A
...
def projectRoot : IO System.FilePath := do
  ...
  let path : System.FilePath := ".lake/packages/cbc-mac-cc"
  if ← (path / "CBCMAC" / "Main.lean").pathExists then return path
  throw (IO.userError
    "the cbc-mac-cc dependency's CBCMAC/Main.lean was not found; run `lake build` first")

/-- Resolve the Lake setup of `path` inside `lakeRoot` from this workspace,
which owns `lakeRoot` as a materialized package.  Running `lake setup-file`
in the dependency's own directory would make Lake treat it as a root
workspace and resolve its dependencies afresh. -/
def lakeSetupFor (lakeRoot path : System.FilePath) : IO ModuleSetup := do
  let source ← IO.FS.realPath (lakeRoot / path)
  let output ← IO.Process.output {
    cmd := "lake"
    args := #["setup-file", source.toString]
  }
