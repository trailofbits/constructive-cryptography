...
unsafe def check : IO Unit := do
  ...
  assertIO (collapsedProof.contains
      ...
      collapsedProof.contains "Following CR18" &&
      collapsedProof.contains "monotone binary output" &&
      collapsedProof.contains "A=(A_1,A_2,\\ldots)" &&
      collapsedProof.contains "A_i=1" &&
      collapsedProof.contains "up to the evaluation of the \\(i\\)-th message" &&
      collapsedProof.contains "sharing a prefix is not counted" &&
      collapsedProof.contains "If \\(A_i=0\\)" &&
      collapsedProof.contains "final round-function inputs distinct" &&
      collapsedProof.contains "CBC outputs are uniform on distinct messages" &&
      collapsedProof.contains "repeated messages receive the same output" &&
      collapsedProof.contains "equation (6.2)" &&
      collapsedProof.contains
        "\\widehat{\\mathsf{CBC}}[B; R] \\mathrel{\\mid\\!\\equiv} V" &&
      collapsedProof.contains "Applying the total-block restriction" &&
      collapsedProof.contains
        "\\theta[B, q]\\cdot \\widehat{\\mathsf{CBC}}[B; R] \\mathrel{\\mid\\!\\equiv} \\theta[B, q]\\cdot V" &&
      collapsedProof.contains "Theorem 4.17" &&
      ...
      collapsedProof.contains "bounds the original distance" &&
