...
This module relates Lanzenberger's fixed `(X,Y)` DDS/PDS presentation directly
to the cumulative query-indexed `RandomSystem` at `Interface.single X Y`.
There is no intermediate ambient observational quotient. Rejection is an
ordinary optional reply and does not stop the observer. The complete attempted
history remains the argument of every later DDS value.
...
abbrev Reply {X : Type u} {Y : Type v} :=
  Ambient.DDC.History.InnerReply
    (Ambient.Interface.single X Y)

def replyAnswer {X : Type u} {Y : Type v}
...
def decodeAnswers {X : Type u} {Y : Type v} :
    Ambient.Transcript (Ambient.Interface.single X Y) →
      Option (List Y)
  | [] => some []
  | reply :: replies =>
...
@[simp]
theorem decodeAnswers_encodeTranscript {X : Type u} {Y : Type v}
    (transcript : System.Transcript X Y) :
...
transcripts. -/
noncomputable def liftDDE {X : Type u} {Y : Type v}
...
theorem transcript_liftDDE_embedDDS {X : Type u} {Y : Type v}
...
theorem literalTranscriptLaw_eq_decode_trLaw
    {X : Type u} {Y : Type v}
    (environment : System.DDE Y X)
    (presentation :
    ...
    (rounds : Nat)
    (enough : PDS.stoppingBound environment presentation.law.1 stops ≤ rounds) :
  ...
  have stable := PDS.trN_succ_eq_of_stoppingBound_le environment
...
def fixedQueryDDE {X : Type u} {Y : Type v}
    (queries : List X) :
    Ambient.DDE (Ambient.Interface.single X Y) :=
  fun observations => queries[observations.length]?

theorem transcriptInputs_fixedQueryDDE {X : Type u} {Y : Type v}
    (system : Ambient.DDS (Ambient.Interface.single X Y))
    (queries : List X) :
...
def finalAccepted {X : Type u} {Y : Type v}
    (observations :
...
noncomputable def domainMembership {X : Type u}
    (domain : Set (List X)) (history : List X) : Bool := by
  classical
  exact decide (history ∈ domain)

theorem finalAccepted_fixedQueryDDE {X : Type u} {Y : Type v}
    (system : System.DDS X Y) (prior : List X) (query : X) :
    finalAccepted
...
theorem finalAcceptedLaw_eq_single {X : Type u} {Y : Type v}
    (presentation :
      RandomSystems.CommonDomain.ProbabilityPresentation X Y)
...
theorem probabilitySupport_nonempty {A : Type u}
...
theorem literalEquivalent_of_ambientEquivalent
    {X : Type u} {Y : Type v}
    {left right :
      RandomSystems.CommonDomain.ProbabilityPresentation X Y}
  ...
  · intro environment admissible
    let rounds := max
      (PDS.stoppingBound environment left.law.1 admissible.1.2)
      (PDS.stoppingBound environment right.law.1 admissible.2.2)
    have leftStable :
        RandomSystems.CommonDomain.ProbabilityPresentation.transcriptLaw
            environment left =
          Distribution.fTransform decodeTranscript
...
def replayFrom {X : Type u} {Y : Type v}
    (environment :
      Ambient.DDE (Ambient.Interface.single X Y)) :
...
theorem replayFrom_append {X : Type u} {Y : Type v}
    (environment :
      Ambient.DDE (Ambient.Interface.single X Y))
...
theorem replayFrom_prefix {X : Type u} {Y : Type v}
    (environment :
      Ambient.DDE (Ambient.Interface.single X Y))
    (history : Ambient.Transcript
      (Ambient.Interface.single X Y))
...
theorem replay_prefix_result {X : Type u} {Y : Type v}
    (environment :
      Ambient.DDE (Ambient.Interface.single X Y))
...
theorem transcriptInputs_prefix {A : Ambient.Interface.{u, v}}
...
@[simp]
theorem transcriptInputs_append_reply
...
def DomainPrefixClosed {X : Type u}
    (domain : Set (List X)) : Prop :=
  ∀ ⦃short long : List X⦄,
    short <+: long → short ≠ [] → long ∈ domain → short ∈ domain

theorem probabilityPresentation_domainPrefixClosed
    {X : Type u} {Y : Type v}
    (presentation :
      RandomSystems.CommonDomain.ProbabilityPresentation X Y) :
...
noncomputable def restrictedQuery {X : Type u} {Y : Type v}
    (domain : Set (List X))
    (environment :
      Ambient.DDE (Ambient.Interface.single X Y))
...
theorem restrictedQuery_some_data {X : Type u} {Y : Type v}
    (domain : Set (List X))
    (environment :
      Ambient.DDE (Ambient.Interface.single X Y))
...
theorem restrictedQuery_prefix {X : Type u} {Y : Type v}
...
noncomputable def restrictDDE {X : Type u} {Y : Type v}
    (domain : Set (List X)) (domainPrefix : DomainPrefixClosed domain)
    (environment :
      Ambient.DDE (Ambient.Interface.single X Y))
...
theorem restrictDDE_halts {X : Type u} {Y : Type v}
    (domain : Set (List X)) (domainPrefix : DomainPrefixClosed domain)
    (environment :
      Ambient.DDE (Ambient.Interface.single X Y))
    (rounds : Nat) :
...
theorem replay_trN_restrictDDE {X : Type u} {Y : Type v}
    (domain : Set (List X)) (domainPrefix : DomainPrefixClosed domain)
    (environment :
      Ambient.DDE (Ambient.Interface.single X Y))
...
theorem restrictDDE_compatible {X : Type u} {Y : Type v}
    (domain : Set (List X)) (domainPrefix : DomainPrefixClosed domain)
    (environment :
      Ambient.DDE (Ambient.Interface.single X Y))
    (rounds : Nat) (system : System.DDS X Y)
...
theorem restrictDDE_stops {X : Type u} {Y : Type v}
    (domain : Set (List X)) (domainPrefix : DomainPrefixClosed domain)
    (environment :
      Ambient.DDE (Ambient.Interface.single X Y))
...
theorem trN_prefix_of_le {X : Type u} {Y : Type v}
    (environment : System.DDE Y X) (system : System.DDS X Y)
...
theorem restrictDDE_stages_eq_of_final_eq
    {X : Type u} {Y : Type v}
    (domain : Set (List X)) (domainPrefix : DomainPrefixClosed domain)
    (environment :
      Ambient.DDE (Ambient.Interface.single X Y))
...
theorem transcript_eq_encode_restricted_of_admitted
    {X : Type u} {Y : Type v}
    (domain : Set (List X)) (domainPrefix : DomainPrefixClosed domain)
    (environment :
      Ambient.DDE (Ambient.Interface.single X Y))
...
theorem ambientTranscript_eq_of_restricted_final_eq
    {X : Type u} {Y : Type v}
    (domain : Set (List X)) (domainPrefix : DomainPrefixClosed domain)
    (environment :
      Ambient.DDE (Ambient.Interface.single X Y))
...
noncomputable def observationOfLiteralTranscript
    {X : Type u} {Y : Type v}
    (domain : Set (List X)) (domainPrefix : DomainPrefixClosed domain)
...
noncomputable def observationOfOptionalLiteralTranscript
    {X : Type u} {Y : Type v}
    (domain : Set (List X)) (domainPrefix : DomainPrefixClosed domain)
    (environment : Ambient.DDE
      (Ambient.Interface.single X Y)) (rounds : Nat) :
...
theorem observationOfOptionalLiteralTranscript_system
    {X : Type u} {Y : Type v}
    (domain : Set (List X)) (domainPrefix : DomainPrefixClosed domain)
    (environment : Ambient.DDE
      (Ambient.Interface.single X Y)) (rounds : Nat)
...
theorem trLaw_embedPDS_eq_map_transcriptLaw_of_domain
    {X : Type u} {Y : Type v}
    (presentation :
      RandomSystems.CommonDomain.ProbabilityPresentation X Y)
    (domain : Set (List X)) (domainPrefix : DomainPrefixClosed domain)
...
theorem trLaw_embedPDS_eq_map_transcriptLaw
    {X : Type u} {Y : Type v}
    (presentation :
      RandomSystems.CommonDomain.ProbabilityPresentation X Y)
    (environment : Ambient.DDE
...
end Internal.Embedding

open Internal.Embedding

theorem embedPDS_equivalent_of_equivalent
    {X : Type u} {Y : Type v}
    {left right :
      RandomSystems.CommonDomain.ProbabilityPresentation X Y}
    (equivalent :
...
theorem advantage_le_embedPDS_advantage
    {X : Type u} {Y : Type v}
    (left right :
      RandomSystems.CommonDomain.ProbabilityPresentation X Y) :
  ...
  refine Probability.sSup_image_univ_le_of_forall _
    (Ambient.PDS.advantage_nonneg _ _) fun environment => ?_
  let rounds := max
    (PDS.stoppingBound environment.1 left.law.1 environment.2.1.2)
    (PDS.stoppingBound environment.1 right.law.1 environment.2.2.2)
  have leftEnough :
      PDS.stoppingBound environment.1 left.law.1 environment.2.1.2 ≤ rounds :=
    le_max_left _ _
  have rightEnough :
      PDS.stoppingBound environment.1 right.law.1 environment.2.2.2 ≤ rounds :=
    le_max_right _ _
  ...
  calc
    ...
    _ ≤ Ambient.PDS.advantage (embedPDS left.law)
        (embedPDS right.law) :=
      Ambient.PDS.statDist_trLaw_le_advantage _ _
        (liftDDE environment.1) rounds

theorem embedPDS_advantage_le_advantage_of_domain_eq
    {X : Type u} {Y : Type v}
    (left right :
      RandomSystems.CommonDomain.ProbabilityPresentation X Y)
  ...
  refine Real.sSup_le ?_ (RandomSystems.PDS.advantage_nonneg _ _)
  ...
  change Probability.statDist
      (Ambient.PDS.trLaw environment rounds (embedPDS left.law))
      (Ambient.PDS.trLaw environment rounds (embedPDS right.law)) ≤
    RandomSystems.PDS.advantage left.law.1 right.law.1
  rw [trLaw_embedPDS_eq_map_transcriptLaw left environment rounds]
  have rightFactor := trLaw_embedPDS_eq_map_transcriptLaw_of_domain right
    left.domain domainPrefix (fun system supported => by
  ...
  calc
    ...
    _ ≤ RandomSystems.PDS.advantage left.law.1 right.law.1 :=
      ...
      le_csSup (RandomSystems.PDS.bddAbove_advantage _ _)
        ⟨⟨literalEnvironment, ⟨⟨leftCompatible, leftStops⟩,
          ⟨rightCompatible, rightStops⟩⟩⟩, Set.mem_univ _, rfl⟩
...
/-- Two normalized partial-system laws with the same displayed domain have
exactly the ambient advantage of their direct optionalizations. -/
theorem advantage_embedPDS_eq_advantage_of_hasDomain
    {X : Type u} {Y : Type v}
    (left right : Distribution.ProbDist (System.DDS X Y))
    {domain : Set (List X)}
    (leftDomain : RandomSystems.PDS.HasDomain left.1 domain)
    (rightDomain : RandomSystems.PDS.HasDomain right.1 domain) :
    Ambient.PDS.advantage (embedPDS left) (embedPDS right) =
      RandomSystems.PDS.advantage left.1 right.1 := by
  let leftPresentation :
      RandomSystems.CommonDomain.ProbabilityPresentation X Y :=
    { law := left
  ...
  have supportNonempty : ∀
    ...
    simp [Distribution.weight] at weightOne
  obtain ⟨leftSystem, leftSupported⟩ := supportNonempty left
  obtain ⟨rightSystem, rightSupported⟩ := supportNonempty right
  have leftDomainEqual : leftPresentation.domain = domain :=
    (leftPresentation.hasDomain leftSystem leftSupported).symm.trans
      (leftDomain leftSystem leftSupported)
  have rightDomainEqual : rightPresentation.domain = domain :=
    (rightPresentation.hasDomain rightSystem rightSupported).symm.trans
      (rightDomain rightSystem rightSupported)
  have domainEqual : leftPresentation.domain = rightPresentation.domain :=
    leftDomainEqual.trans rightDomainEqual.symm
  exact le_antisymm
    (embedPDS_advantage_le_advantage_of_domain_eq
      leftPresentation rightPresentation domainEqual)
    (advantage_le_embedPDS_advantage leftPresentation rightPresentation)
...
/-- Map normalized common-domain presentation data directly to the sole
cumulative query-indexed random-system carrier by optionalizing its partial
DDSs. This is presentation interpretation, not a second ambient carrier. -/
...
/-- Embed a normalized common-domain random system directly in the cumulative
query-indexed random-system carrier. -/
...
noncomputable instance : MetricSpace (ProbabilityRandomSystem X Y) :=
  MetricSpace.induced toAmbient toAmbient_injective inferInstance

@[simp]
theorem dist_eq_dist_toAmbient
    (left right : ProbabilityRandomSystem X Y) :
    dist left right = dist (toAmbient left) (toAmbient right) :=
  rfl

@[simp]
theorem edist_eq_edist_toAmbient
    (left right : ProbabilityRandomSystem X Y) :
    edist left right = edist (toAmbient left) (toAmbient right) := by
  rw [edist_dist, edist_dist, dist_eq_dist_toAmbient]
...
theorem dist_ofPresentation_of_domain_eq
    (left right : ProbabilityPresentation X Y)
    (domainEqual : left.domain = right.domain) :
    dist (ofPresentation left) (ofPresentation right) =
      RandomSystems.CommonDomain.Presentation.Adv
        (D := left.domain)
        ⟨left.toPresentation, rfl⟩
        ⟨right.toPresentation, domainEqual.symm⟩ := by
  rw [dist_eq_dist_toAmbient, toAmbient_ofPresentation,
    toAmbient_ofPresentation, ProbabilityPresentation.toAmbient,
    ProbabilityPresentation.toAmbient,
    Ambient.RandomSystem.dist_ofPDS_eq]
  exact RandomSystems.CommonDomain.advantage_embedPDS_eq_Adv_of_domain_eq
    left right domainEqual

/-- The extended distance is the canonical nonnegative-real embedding of
Lanzenberger's real-valued distinguishing advantage. -/
    ...
    edist (ofPresentation left) (ofPresentation right) =
      ENNReal.ofReal
        (RandomSystems.CommonDomain.Presentation.Adv
          ...
          ⟨left.toPresentation, rfl⟩
          ⟨right.toPresentation, domainEqual.symm⟩) := by
  rw [edist_dist, dist_ofPresentation_of_domain_eq left right domainEqual]
