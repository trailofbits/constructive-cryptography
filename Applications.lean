...
also imports `Applications.Frost`, which imports `ConstructiveCryptography.MultipartyComputation`. The `Applications`
Lake target additionally owns `Applications.Sponge`, which imports
`AbstractCryptography.Algebra.Star`. Dependencies point from applications into the
theory layers, never back.
-/
